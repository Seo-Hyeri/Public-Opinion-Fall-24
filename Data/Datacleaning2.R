...
ESS <- read.spss("Data/ESS1-9.sav")

es.df.clean <- function(x){
  esx <- as.data.frame(ESS)
  esx <- esx %>% dplyr::select(essround, # REQUIRED: essround
  ...
  esx <- esx %>% dplyr::select(-(start:end))
}

ess_clean<- es.df.clean(ESS)

lapply(ESS, FUN=es.df.clean)
ess2<-as.data.frame(ESS)
a<- ess2 %>% dplyr::select(essround)
  %>% 
    as.data.frame()
  # find FIRST country-specific vote variable
  start <- head(grep("prtv", colnames(esx)), n=1)
...
ess_clean <- lapply(ESS, FUN=es.df.clean)
