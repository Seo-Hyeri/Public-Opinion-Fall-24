...
# CHES (modify destfile if you want to include a particular location)
download.file("https://www.chesdata.eu/s/1999-2019_CHES_dataset_meansv3-x7lr.csv",
              destfile = "ches.csv")
...
ches_pf <- ches %>% 
  left_join(pf, by = c("party_id" = "dataset_party_id"))

# run https://github.com/sophieehill/ess-cumulative/blob/master/build_cumulative_core_ESS.R

ess_ches <- essx %>% 
  left_join(ches, by = "partyfacts_id") # galtan
